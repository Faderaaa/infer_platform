...
def inferSigImg(model, frame):
    ...
    input_feed = model.get_input_feed(frame.astype(np.float32))
    pred = model.onnx_session.run(None, input_feed)[0]
    return dataHandle.analyzeInferResults(model.modelName, pred)
...
if __name__ == "__main__":
    ...
    image = image / 255.0
    image = np.resize(image, (1, 3, 640, 640))
    start = time.time()
    infer = inferSigImg(model, image)
    print(infer.shape)
    print("fps is {}".format(int(1 / (time.time() - start))))

    # output, or_img = model.inference('data/images/img.png')
    # start = time.time()
    # output, or_img = model.inference('/home/hubu/Documents/data/yolo_data/bus.jpg')
    # print("fps is {}".format(int(1 / (time.time() - start))))
    ...
    # cv2.imwrite('/home/hubu/Documents/data/666.jpg', or_img)
